...
const Establishment = () => {
    ...
    useEffect(() => {
        if (item) {
            let calculatedPoints = ((item.scoreFood + item.scoreService + item.scoreAtmosphere + item.scoreInterier) / 4).toFixed(1)
            calculatedPoints = calculatedPoints > 5 ? 5 : calculatedPoints < 0 ? 0 : calculatedPoints
